...
function generate_mtech_is_degree(
...
// First Hindi Details
    ...
    $pdf->Ln();
    $pdf->Cell(0,0,'fu/kkZfjr vgZrk,a lQyrkiwoZd iw.kZ djus ij','','','C');
    $pdf->Ln();
    $pdf->SetFont('kurti-dev-040-bold','',16);
    ...
    $pdf->Ln();
    $pdf->SetFont('kurti-dev-040-bold','',16);
    $pdf->Cell(0,$LINE_SPACING,"izkSS/kksfxdh vf/kLukrd",0,'','C');
    $pdf->Ln();
    $pdf->SetFont('Kruti_Dev_100','',14);
    $pdf->Cell(410,0,'¼','','','C');
    $pdf->Ln();
    $pdf->SetFont('kurti-dev-040-bold','',16);
    $pdf->Cell(540,0,'bUQksjes\'ku lsD;wfjVh',0,'','C');
    $pdf->SetFont('Kruti_Dev_100','',14);
    $pdf->Cell(-435,0,'½','','','C');
    ...
    $pdf->Cell(-465,$LINE_SPACING,'½','','','C');
//    $pdf->Text($pageWidth/2-40,260,'¼'.$startYear.'&'.$endYear);
//    $pdf->Text($pageWidth/2+20,260,'½');

    $pdf->SetFont('Kruti_Dev_010','',16);
    $pdf->Ln();
    $pdf->Cell(0,0,'dh mikf/k','','','C');
    $pdf->Ln();
    $pdf->Cell(0,$LINE_SPACING,'iznku djrk gS','','','C');
...
//English Typing
    ...
    $pdf->Ln();
    $pdf->SetFont('times','B',12);
    $pdf->Cell(0,0,'MASTER OF TECHNOLOGY (INFORMATION SECURITY)','','','C');
    $pdf->SetFont('times','',12);
    $pdf->Ln();
